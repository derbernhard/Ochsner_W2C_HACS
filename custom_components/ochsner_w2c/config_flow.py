...
class OchsnerW2CConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input:
            try:
                await W2CApi(self.hass, user_input[CONF_W2C_HOST], user_input[CONF_USERNAME], user_input[CONF_PASSWORD]).read_oid(OIDS[0])
            except W2CApiError:
                errors['base'] = 'cannot_connect'
            else:
                await self.async_set_unique_id(user_input[CONF_W2C_HOST].lower())
                self._abort_if_unique_id_configured()
                return self.async_create_entry(title=f"Ochsner W2C ({user_input[CONF_W2C_HOST]})", data=user_input)
        schema = vol.Schema({
